...
leaderboards for the selected team's flair. Fanbase negativity rankings
table is always visible at the bottom.
"""
...
has_team = selected_team != "Select your team..."

if not has_team:
    st.info("Pick a team from the dropdown to explore your fanbase's sentiment.")

# ---------------------------------------------------------------------------
# Team-specific content (only when a team is selected)
# ---------------------------------------------------------------------------
...
if has_team:
    ...
    team_flair = team_flair_all[
        team_flair_all["comment_count"] >= median_cc
    ].copy()
    ...
    top_discussed = (
        ...
        .head(10)
        .sort_values("comment_count", ascending=True)
    )
...
# ---------------------------------------------------------------------------
# Fanbase negativity rankings (always visible)
# ---------------------------------------------------------------------------

st.markdown("---")
st.subheader("Fanbase Negativity Rankings")

rankings = (
    team_overall.sort_values("neg_rate", ascending=False)
...
if has_team:
    rankings["team_display"] = rankings["team"].apply(
        lambda t: f"➤ {t}" if t == selected_team else t
    )
else:
    rankings["team_display"] = rankings["team"]

# Convert 0-1 rates to 0-100 for printf-style formatting
rankings["neg_rate_pct"] = rankings["neg_rate"] * 100
rankings["pos_rate_pct"] = rankings["pos_rate"] * 100

st.dataframe(
    rankings,
    column_config={
        "logo_url": st.column_config.ImageColumn("", width="small"),
        "team_display": st.column_config.TextColumn("Team"),
        ...
        "net_sentiment": st.column_config.NumberColumn(
            "Net Sentiment", format="%+.3f"
        ),
        "comment_count": st.column_config.NumberColumn(
            "Total Comments", format="%d"
        ),
    },
    column_order=[
        "logo_url",
